#!/usr/bin/env python3
"""
Step 3: Page-Level Indexing using VLM (Gemini 2.0 Flash)
=========================================================
"""
...
REQUEST_DELAY = 4.5  # seconds between requests (free tier: 15 RPM)
MAX_RETRIES = 3
...
def load_api_key() -> str:
    ...
    # Try loading from .env file in current directory or parent
...
def classify_page(image_path: str, api_key: str) -> dict:
    """
    Send an image to Gemini API and get page classification results.
    Returns a dict with has_image, has_text, count_of_image, img_layout.
    """
...
def _parse_response(text: str) -> dict:
    ...
    try:
        ...
        if count > 1 and layout == "sgl":
            layout = "td"  # default to top-down for multiple images
...
def process_book(image_dir: str, book_id: str, output_csv: str,
                 api_key: str, limit: int = 0, resume: bool = False,
                 delay: float = 4.5):
    """
    ...
    Args:
        ...
        limit:      max number of pages to process (0 = all)
    ...
    with open(output_csv, mode, newline="", encoding="utf-8") as f:
        ...
        for i, img_file in enumerate(files):
            page_id = img_file.stem  # e.g. "gy_mg_10"
            ...
            result = classify_page(str(img_file), api_key)
...
def main():
    parser = argparse.ArgumentParser(
        description="Step 3: Page-level indexing using Gemini VLM",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python step3_index.py ./output/gy_mg/ --book-id gy_mg
  python step3_index.py ./output/gy_mg/ --book-id gy_mg --limit 10
  python step3_index.py ./output/gy_mg/ --book-id gy_mg --resume
  python step3_index.py ./output/gy_mg/ --book-id gy_mg -o results/gy_mg.csv
...
The script reads GEMINI_API_KEY from:
  ...
  2. A .env file in the current or parent directory
    ...
    parser.add_argument("--resume", action="store_true",
                        help="Resume from where we left off (skip already-done pages)")
    parser.add_argument("--delay", type=float, default=REQUEST_DELAY,
                        help="Seconds between API requests (default: 4.5)")
    ...
    log.info("=" * 60)
    log.info("Step 3: Page-Level Indexing (Gemini VLM)")
    log.info("=" * 60)
